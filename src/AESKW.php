...
trait AESKW
{
    ...
    {
        $A = self::get_initial_value($key, $padding_enabled);
        self::check_key_size($key, $padding_enabled);
        ...
        if ($N === 1) {
            ...
            for ($j = 0; $j <= 5; ++$j) {
                for ($i = 1; $i <= $N; ++$i) {
                    ...
                    $t = $i + $j * $N;
                    $A = self::to_x_bits(64, $t) ^ self::get_msb($B);
                    $R[$i - 1] = self::get_lsb($B);
                }
            }
            $C = array_merge([$A], $R);
        }
    ...
    public static function unwrap(string $kek, string $key, bool $padding_enabled = false): string
    {
        ...
        if ($N === 2) {
            ...
            for ($j = 5; $j >= 0; --$j) {
                for ($i = $N - 1; $i >= 1; --$i) {
                    $t = $i + $j * ($N - 1);
                    $B = self::decrypt($kek, (self::to_x_bits(64, $t) ^ $A) . $R[$i]);
                    $A = self::get_msb($B);
                    $R[$i] = self::get_lsb($B);
                }
            }
        ...
        if (self::check_initial_value($unwrapped, $padding_enabled, $A) === false) {
     ...
     */
    private static function get_initial_value(string &$key, bool $padding_enabled): string
    {
        ...
        $iv = hex2bin('A65959A6') . self::to_x_bits(32, $MLI);
        $n = (int) ceil($MLI / 8);
        $key = str_pad($key, 8 * $n, "\x00", STR_PAD_RIGHT);
        return $iv;
    }
    private static function check_initial_value(string &$key, bool $padding_enabled, string $iv): bool
    {
        // RFC3394 compliant
        ...
        if ($rfc5649Prefix === false || !hash_equals($rfc5649Prefix, self::get_msb($iv))) {
            return false;
        }
        $n = mb_strlen($key, '8bit') / 8;
        $MLI = (int) hexdec(bin2hex(ltrim(self::get_lsb($iv), "\x00")));
    ...
    private static function check_key_size(string $key, bool $padding_enabled): void
    {
        if ($key === '') {
    ...
    private static function to_x_bits(int $bits, int $value): string
    {
    ...
    private static function get_msb(string $value): string
    {
        return mb_substr($value, 0, mb_strlen($value, '8bit') / 2, '8bit');
    }
    private static function get_lsb(string $value): string
    {
        return mb_substr($value, mb_strlen($value, '8bit') / 2, null, '8bit');
    }
    private static function encrypt(string $kek, string $data): string
    {
        $result = openssl_encrypt($data, self::get_method(), $kek, OPENSSL_ZERO_PADDING | OPENSSL_RAW_DATA);
        if ($result === false) {
    ...
    {
        $result = openssl_decrypt($data, self::get_method(), $kek, OPENSSL_ZERO_PADDING | OPENSSL_RAW_DATA);
        if ($result === false) {
